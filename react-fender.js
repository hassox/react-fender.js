...
function wrapField(name, isRequired, child, remover, errorComponent, isError) {
  ...
  return (
    React.DOM.div({className: "proto-field " + isError, 'data-error': errorComponent}, 
      removeButton, 
      React.DOM.label(null, requiredMarker(isRequired), " ", name), 
      child
    )
  );
}
...
var StringField = React.createClass({displayName: 'StringField',
  ...
  render: function() {
    ...
    if(this.state.textarea) {
      child = ( React.DOM.textarea({ref: data.id, placeholder: data.name, required: required, onChange: this.provideValue}, value) );
    } else {
      child = ( React.DOM.input({type: "text", ref: data.id, placeholder: data.name, value: value, required: required, onChange: this.provideValue}) );
    }
    return child;
  }
});
...
var EnumField = React.createClass({displayName: 'EnumField',
  ...
  render: function() {
    ...
    return ( React.DOM.select({ref: data.id, value: value, onChange: this.provideValue}, options) );
  }
});
...
var BoolField = React.createClass({displayName: 'BoolField',
  ...
  render: function() {
    ...
    return (
      React.DOM.input({ref: data.id, type: "checkbox", value: "true", checked: value, onChange: this.provideValue})
    );
  }
});
...
var MessageField = React.createClass({displayName: 'MessageField',
  ...
  render: function() {
    ...
    if(!msg) {
      return ( React.DOM.span({onClick: this.createNew}, "+") );
    } else {
  ...
  render: function() {
    ...
    return (
      React.DOM.div({className:  "proto-field " + (isError || ''), 'data-errors': errorComponent}, 
        React.DOM.label(null, requiredMarker(required), " ", data.name, " ", React.DOM.span({className: "add-new", onClick: this.addNew}, "+")), 
...
var MessageForm = React.createClass({displayName: 'MessageForm',
  ...
  render: function() {
    ...
    return (
      React.DOM.form({onSubmit: this.handleSubmit, className: "messageForm"}, 
...
},{"stream":undefined}],"through":[function(require,module,exports){
var Stream = require('stream')
